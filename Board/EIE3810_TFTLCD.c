...
void EIE3810_TFTLCD_Init(void)
{
  ...
  RCC->APB2ENR|=1<<3;     	//Enable PORTB clock
	...
	FSMC_Bank1->BTCR[6]|=1<<12;		//Set FSMC registers bit 12 to 1
	FSMC_Bank1->BTCR[6]|=1<<14;		//Set FSMC registers bit 14 to 1
	FSMC_Bank1->BTCR[6]|=1<<4; 		//Set FSMC registers bit 4 to 1
						    
	FSMC_Bank1->BTCR[7]|=0<<28;		//Set FSMC registers bit 28 to 0
	FSMC_Bank1->BTCR[7]|=1<<0; 		//Set FSMC registers bit 0 to 1
	FSMC_Bank1->BTCR[7]|=0XF<<8;  	//Set FSMC registers bit 8
	
	FSMC_Bank1E->BWTR[6]|=0<<28; 	//SRAM/NOR-Flash write timing registers
	FSMC_Bank1E->BWTR[6]|=0<<0;		//SRAM/NOR-Flash write timing registers
 	FSMC_Bank1E->BWTR[6]|=3<<8; 	//SRAM/NOR-Flash write timing registers
	
	FSMC_Bank1->BTCR[6]|=1<<0;		//Set FSMC registers bit 0 to 1
	
	for (u32 i=0;i<510000;i++);
...
void EIE3810_TFTLCD_SevenSegment(u16 start_x, u16 start_y, u8 digit, u16 color)
{
	if(digit == 1)
	{
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+65), 55, color); //verical right buttom
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+130), 55, color); //vertical right up
	}
	else if(digit == 2)
	{
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+10), 10, color); //horizontal bottom
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+75), 10, color); //horizontal middle
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+140), 10, color); //horizontal up
		EIE3810_TFTLCD_FillRectangle(start_x, 10, 800-(start_y+65), 55, color); //verical left buttom
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+130), 55, color); //verical right up
	}
	else if(digit == 3)
	{
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+10), 10, color); //horizontal bottom
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+75), 10, color); //horizontal middle
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+140), 10, color); //horizontal up
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+65), 55, color); //vertical right buttom
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+130), 55, color);  //vertical right up
	}
	else if(digit == 4)
	{
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+75), 10, color); //horizontal middle
		EIE3810_TFTLCD_FillRectangle(start_x, 10, 800-(start_y+130), 55, color);  //vertical left up
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+65), 55, color); //vertical right buttom
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+130), 55, color); //vertical right up
	}
	else if(digit == 5)
	...
	else if(digit == 6)
	{
		...
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+75), 10, color); //horizontal middle
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+140), 10, color); //horizontal up
		EIE3810_TFTLCD_FillRectangle(start_x, 10, 800-(start_y+65), 55, color);  //vertical left down
		EIE3810_TFTLCD_FillRectangle(start_x, 10, 800-(start_y+130), 55, color);  //vertical left up
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+65), 55, color); //vertical right buttom
	}
	else if(digit == 7)
	...
	else if(digit == 8)
	{
		...
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+75), 10, color); //horizontal middle
		EIE3810_TFTLCD_FillRectangle(start_x+10, 55, 800-(start_y+140), 10, color); //horizontal up
		EIE3810_TFTLCD_FillRectangle(start_x, 10, 800-(start_y+65), 55, color);  //vertical left buttom
		EIE3810_TFTLCD_FillRectangle(start_x, 10, 800-(start_y+130), 55, color);  //vertical left up
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+65), 55, color); //vertical right buttom
		EIE3810_TFTLCD_FillRectangle(start_x+65, 10, 800-(start_y+130), 55, color); //vertical right up
	}
...
void EIE3810_TFTLCD_ShowChar(u16 x, u16 y, u8 ASCII, u16 color, u16 bgcolor)
{
	int disp = ASCII-32;
	u32 index = 0;
	EIE3810_TFTLCD_WrCmd(0x2A00); //lines 631 to 649 are similar to fill rectangle function
	EIE3810_TFTLCD_WrData(x >> 8);
	EIE3810_TFTLCD_WrCmd(0x2A01);
	EIE3810_TFTLCD_WrData(x & 0xFF);
	EIE3810_TFTLCD_WrCmd(0x2A02);
	...
	for (u32 j = 0; j < 8; j++){
		...
		{
			u32 actual = index * 2; //take the even only 0, 2, .. 14
			unsigned char arr = asc2_1608[disp][actual]; //go to the wanted hexadeciaml value
			if((arr >> p)&0x01){
				EIE3810_TFTLCD_WrData(color); //if 1, fill in using the color
			}
			else {
				EIE3810_TFTLCD_WrData(bgcolor); //if 0, fill in using the bgcolor
			}
		}
	}
	for (u32 j = 0; j < 8; j++){
		...
		for(index = 0; index < 8; index++)
		{
			u32 actual = index * 2 + 1; //take the odd only 1, 3, .. 15
			unsigned char arr = asc2_1608[disp][actual]; //go to the wanted hexadeciaml value
			if((arr >> p)&0x01){
				EIE3810_TFTLCD_WrData(color); //if 1, fill in using the color
			}
...
{
	u32 index = 0;
	EIE3810_TFTLCD_WrCmd(0x2A00); //these lines 685 to 703 are similar to fill rectangle
	EIE3810_TFTLCD_WrData(0 >> 8);
	EIE3810_TFTLCD_WrCmd(0x2A01);
	EIE3810_TFTLCD_WrData(0 & 0xFF);
	EIE3810_TFTLCD_WrCmd(0x2A02);
	EIE3810_TFTLCD_WrData((480 - 1) >> 8);
	EIE3810_TFTLCD_WrCmd(0x2A03);
	EIE3810_TFTLCD_WrData((480 - 1) & 0xFF);
	...
	EIE3810_TFTLCD_WrData(0 & 0xFF);
	EIE3810_TFTLCD_WrCmd(0x2B02);
	EIE3810_TFTLCD_WrData((800 - 1) >> 8);
	EIE3810_TFTLCD_WrCmd(0x2B03);
	EIE3810_TFTLCD_WrData((800 - 1) & 0xFF);
	
	EIE3810_TFTLCD_WrCmd(0x2C00);
	for(index = 0; index < 480*800; index++) //for the given size, color all pixels with the given color
	{
		EIE3810_TFTLCD_WrData(color);		
	}
}
...
void ShowCharALine(u16 x, u16 y, char*string, u16 color, u16 bgcolor)
{
	u8 i=0;
	while (string[i]!=0x00)
	{
...
void ShowCharALine2412(u16 x, u16 y, char*string, u16 color, u16 bgcolor)
{
	u8 i=0;
	while (string[i]!=0x00)
	{
